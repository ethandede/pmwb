...
# ---------------------------------------------------------------------------
# PJM Solar / Power Price Signal
# ---------------------------------------------------------------------------

def get_pjm_solar_signal(
    lat: float, lon: float,
    hub_key: str = "",
    solar_sensitivity: float = 0.15,
    hours_ahead: int = 24,
    ...
    """Solar irradiance + PJM price -> fair-price trading signal.

    Uses per-hub solar sensitivity and load forecast to estimate
    whether the current hub price is above or below fair value.
    """
    from config import (
        VISUAL_CROSSING_API_KEY, PJM_SEASONAL_NORMS,
        PJM_LOAD_SENSITIVITY,
    )

    # 1. Seasonal norms for current month
    month = datetime.now(timezone.utc).month
    norms = PJM_SEASONAL_NORMS.get(month)
    if norms is None:
        try:
            from alerts.telegram_alert import send_alert
            send_alert("PJM Norms Missing", f"No seasonal norms for month {month}",
                       dedup_key="pjm_norms_missing")
        except Exception:
            pass
        norms = {"solar": 12.0, "load": 85_000}
    norm_solar = norms["solar"]
    norm_load = norms["load"]
    ...
    # 3. PJM market data
    if pjm_data is not None:
        hub_price = float(pjm_data.get("hub_price", pjm_data.get("price", 35.0)))
        actual_solar_mw = float(pjm_data.get("solar_mw", 0.0))
        load_forecast = float(pjm_data.get("load_forecast", norm_load))
    else:
        hub_price = 35.0
    ...
    if contract_hour > 0 and dam_price > 0:
        ...
        return {
            ...
            "model_prob": model_prob,
            "expected_solrad_mjm2": round(safe_expected_solrad, 1),
            "current_pjm_price": round(hub_price, 1),
            "actual_solar_mw": round(actual_solar_mw, 0),
            "confidence": confidence,
        }
    ...
    # 4. Fair price model (daily edge — backward-compat path)
    solar_impact = solar_sensitivity * (norm_solar - expected_solrad) / norm_solar
    load_impact = PJM_LOAD_SENSITIVITY * (load_forecast - norm_load) / norm_load
    edge = round(solar_impact + load_impact, 4)
