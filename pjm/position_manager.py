"""PJM position manager — evaluate/fortify/exit logic for directional positions.

Evaluates open positions against current signals to decide hold/fortify/exit.
"""

from config import (
    PJM_FORTIFY_EDGE_INCREASE,
    PJM_EXIT_EDGE_DECAY,
    PJM_MAX_POSITIONS_PER_HUB,
    PJM_PAPER_BANKROLL,
)
...
def evaluate_pjm_position(position: dict, current_signal: dict) -> dict:
    ...
    if entry_edge > 0:
        ...
        if edge_ratio < PJM_EXIT_EDGE_DECAY:
            return {
                "action": "exit",
                "reason": f"Edge decay: {current_edge:.2f} / {entry_edge:.2f} = {edge_ratio:.2f} < {PJM_EXIT_EDGE_DECAY}",
            }

    # Fortify check: if current edge exceeds entry edge by threshold -> fortify
    edge_increase = current_edge - entry_edge
    if edge_increase > PJM_FORTIFY_EDGE_INCREASE and pos_signal == cur_signal:
        return {
            "action": "fortify",
            "reason": f"Edge increased by {edge_increase:.2f} (>{PJM_FORTIFY_EDGE_INCREASE})",
        }
...
def run_pjm_manager():
    ...
    current_price = market_data.get("price", 35.0)

    # Expire old positions
    expire_positions(current_price)

    # Scan current signals
    current_signals = scan_all_pjm_hubs()
    ...
    for pos in positions:
        ...
        if result["action"] == "exit":
            close_position(
                pos["id"],
                exit_price=current["current_pjm_price"],
                exit_signal=current["signal"],
                reason=result["reason"],
            )
        elif result["action"] == "fortify":
            # Open additional position in same direction
            open_position(current, bankroll=PJM_PAPER_BANKROLL)

        actions.append(result)

    if actions:
        print(f"  PJM manager: {len(actions)} positions evaluated")
        for a in actions:
