"""CAISO paper trading engine — directional solar-price signals.

Directional contracts: SHORT (expect price drop) or LONG (expect price rise).
Positions expire after CAISO_POSITION_TTL_HOURS and are settled at current price.
"""
...
from config import (
    ...
    CAISO_POSITION_TTL_HOURS,
)
from risk.position_limits import check_limits

CAISO_PAPER_DB = "data/caiso_paper.db"

_SCHEMA = """
CREATE TABLE IF NOT EXISTS caiso_positions (
    ...
    current_caiso_price REAL NOT NULL,
    opened_at TEXT NOT NULL,
    expires_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS caiso_trades (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hub TEXT NOT NULL,
    hub_name TEXT NOT NULL,
    city TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS caiso_scan_cache (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    hub TEXT NOT NULL,
    hub_name TEXT NOT NULL,
    signal TEXT NOT NULL,
...
def _init_db():
    os.makedirs(os.path.dirname(CAISO_PAPER_DB) or ".", exist_ok=True)
    if os.path.exists(CAISO_PAPER_DB):
        bak = CAISO_PAPER_DB + ".bak"
        try:
            shutil.copy2(CAISO_PAPER_DB, bak)
        except OSError:
            pass
    conn = sqlite3.connect(CAISO_PAPER_DB)
...
def open_position(hub_signal: dict, bankroll: float, max_size: float = None) -> dict | None:
    ...
    hub_signal keys required:
        hub, hub_name, signal, edge, confidence, current_caiso_price
    """
    conn = _conn()
    ...
    hub_count = conn.execute(
        "SELECT COUNT(*) FROM caiso_positions WHERE hub = ?", (hub,)
    ).fetchone()[0]
    if hub_count >= _config.CAISO_MAX_POSITIONS_PER_HUB:
        conn.close()
        return None

    # Check total limit
    total_count = conn.execute("SELECT COUNT(*) FROM caiso_positions").fetchone()[0]
    if total_count >= _config.CAISO_MAX_POSITIONS_TOTAL:
        conn.close()
        return None

    # Kelly sizing
    edge = hub_signal["edge"]
    effective_bankroll = min(bankroll, CAISO_PAPER_BANKROLL)
    size = min(
    ...
    # Check risk limits
    total_exposure = sum(
        r[0] for r in conn.execute("SELECT size_dollars FROM caiso_positions").fetchall()
    )
...
def close_position(position_id: int, exit_price: float, exit_signal: str, reason: str):
    ...
    """
    conn = _conn()
    row = conn.execute("SELECT * FROM caiso_positions WHERE id = ?", (position_id,)).fetchone()
    if row is None:
        conn.close()
        return
    ...
    conn.execute(
        """INSERT INTO caiso_trades
           (hub, hub_name, city, signal, edge, confidence,
            entry_price, exit_price, size_dollars, pnl,
            expected_solrad_mjm2, actual_solar_mw, current_caiso_price,
        ...
        (
            ...
            row["current_caiso_price"],
            row["opened_at"], closed_at, exit_signal, reason,
        ),
    )
    conn.execute("DELETE FROM caiso_positions WHERE id = ?", (position_id,))
    conn.commit()
    conn.close()
...
def expire_positions(current_price: float):
    ...
    conn = _conn()
    now = datetime.now(timezone.utc)
    positions = conn.execute("SELECT * FROM caiso_positions").fetchall()
    ...
    for row in positions:
        ...
        if now >= expires_at:
            ...
            conn.execute("DELETE FROM caiso_positions WHERE id = ?", (row["id"],))
...
def get_open_positions() -> list:
    conn = _conn()
    rows = conn.execute("SELECT * FROM caiso_positions ORDER BY opened_at DESC").fetchall()
    conn.close()
...
def get_trade_history(limit: int = 50) -> list:
    conn = _conn()
    rows = conn.execute(
        "SELECT * FROM caiso_trades ORDER BY closed_at DESC LIMIT ?", (limit,)
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_paper_summary() -> dict:
    conn = _conn()
    positions = conn.execute("SELECT * FROM caiso_positions").fetchall()
    trades = conn.execute("SELECT pnl FROM caiso_trades").fetchall()
    conn.close()
...
def write_scan_cache(signals: list):
    ...
    conn.execute("DELETE FROM caiso_scan_cache")
    for sig in signals:
        conn.execute(
            """INSERT INTO caiso_scan_cache
               (hub, hub_name, signal, edge, expected_solrad_mjm2,
                current_caiso_price, actual_solar_mw, confidence, scanned_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                sig["hub"], sig["hub_name"],
                sig.get("signal", ""), sig["edge"],
                sig.get("expected_solrad_mjm2"),
                sig.get("current_caiso_price", 0.0),
...
def get_cached_signals() -> list:
    conn = _conn()
    rows = conn.execute("SELECT * FROM caiso_scan_cache ORDER BY hub").fetchall()
    conn.close()
    return [dict(r) for r in rows]
